...
const STR = {
  zh: {
    ...
    auth: {
      ...
      logout: "登出",
      notConfigured: "還沒設定資料庫連線（VITE_SUPABASE_URL、VITE_SUPABASE_ANON_KEY）。請照部署指南設定後重新部署。",
  ...
  en: {
    ...
    auth: {
      ...
      logout: "Sign out",
      notConfigured: "Database connection isn't configured yet (VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY). Follow the deployment guide and redeploy.",
...
export default function PetJournal() {
  ...
  else body = <List pets={pets} storageOk={storageOk} email={session.user.email} onLogout={logout} onOpen={(id) => setView({ name: "detail", id })} onAdd={() => setView({ name: "form" })} />;
  ...
  return (
    <>
      <header className="pp-top">
        ...
        <button className="pp-link" onClick={onLogout}>{L.auth.logout}</button>
        <LangToggle />
      </header>
