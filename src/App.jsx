...
/* ---- 玩伴清单 ---- */
.pp-mate{position:relative;background:var(--card);border-radius:3px;box-shadow:0 2px 6px rgba(59,48,36,.16);margin:0 16px 16px;padding-bottom:2px;}
...
const STR = {
  ...
  en: {
    ...
    mates: {
      ...
      score: (n) => `Match ${n}`,
      aiNote: "Scores and reasons are generated by AI from both profiles.",
      r: {
...
/* ---- AI 配对：提示词（和 supabase/functions/match-playmates/index.ts 一致，改一边记得改另一边） ---- */
function buildMatchPrompt(me, cands) {
  const brief = (p) => {
    const a = ageParts(p.birthday);
    return { id: p.id, name: p.name, breed: breedLabel(p.species, p.breed, "en") || "unknown", sex: p.gender || "unknown",
      age_months: a ? a.y * 12 + a.m : null, weight_kg: Number(p.weightKg) || null, neutered: !!p.neutered, note: (p.note || "").trim() || undefined };
  };
  return `You are the playmate-matching assistant in a pet journal app. All pets below are ${me.species}s living in the same city.

MY PET: ${JSON.stringify(brief(me))}
CANDIDATES: ${JSON.stringify(cands.map(brief))}

Score each candidate 0-100 as a playmate for MY PET. Consider: life stage compatibility (puppy/kitten, adult, senior), size and weight gap, energy level implied by age and breed, sex and neuter status (two intact males score lower), temperament hints in notes, and safety. ${me.species === "cat" ? "Cats are territorial and rarely benefit from meeting; score for how compatible the two households would be for exchanging cat-care tips or a cautious future introduction." : ""}
Pick the single best candidate.

Return ONLY a JSON object, no markdown, no explanation:
{"best_id":"<id>","scores":[{"id":"<id>","score":0-100}],"reasons":{"zh":["3 short bullet phrases in Simplified Chinese, each under 18 characters, why best_id fits"],"en":["the same 3 phrases in English, each under 10 words"]}}`;
}
...
/* Netlify 版：交给 Supabase Edge Function「match-playmates」（在资料库里挑候选、让 AI 打分，只有最佳配对带 Email）。
   AI 失败就退回资料库的规则版 find_playmates()。 */
          ...
          <div className={`pp-mate${o.isMatch ? " match" : ""}`} key={o.id}>
            ...
            {o.isMatch && (
              <>
                ...
                {o.aiScored && <div className="pp-src" style={{ padding: "0 16px 10px" }}>{M.aiNote}</div>}
